...
Production-grade MCP server for Instagram. **82 tools** — 22 anonymous, 59 authenticated, 1 auto-mode. Built on `curl_cffi` with Chrome TLS impersonation, adaptive rate limiting, smart caching, multi-account pool, and challenge/2FA resolver.
...
|------|--------|-------------|-------|
| Anonymous | 🌐 | None | 22 |
| Authenticated | 🔐 | `cookies.json` with valid Instagram session | 59 |
...
| `instagram_dm_share_post` | Share a post into a thread or to a user |
| `instagram_dm_media_messages` | List media messages (photos, videos, shared posts) in a thread |
...
**Do I need to log in?**  
No. 22 tools work anonymously with no credentials. 59 tools require `cookies.json`. `instagram_hashtag` auto-switches based on whether cookies are present.
